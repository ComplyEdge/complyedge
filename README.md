...
Runtime EU AI Act enforcement for AI systems. Not a scanner — runs in production, on every request.
...
rules/rego/           28 OPA/Rego policies (EU AI Act Article 5, 6, 50, GPAI)
...
53 YAML rules + 28 OPA/Rego policies across 4 jurisdictions:

| Jurisdiction | Rules | Regulations |
|---|---|---|
| **EU** | 36 YAML + 28 Rego | EU AI Act Articles 4–6, 9–10, 12–16, 26–27, 50, 53, GPAI, GDPR |
...
**Layer 1 — Deterministic (hot path):** 28 OPA/Rego policies evaluate every request. Blocked prompts return with a legal citation in tens of milliseconds — 38–100ms (median 62ms) across the OPA-blocked prompts in our benchmark. Binary pass/block, no LLM on the hot path. (TrustLint applies the same regex corpus offline for CI use.)
