...
**Status:** Methodology v1.1 (2026-07-05) — the deterministic corpus is now **10 rules** across all four sub-categories under `rules/regulations/universal/prompt_security/`, and the **Layer 2 IPI behavioural clause** (`art15_ipi_behavioral_subversion` in `services/api/layer2_service.py`) is shipped. GPAI-benchmark IPI scoring is the remaining follow-up.
...
1. **A 2026 frontier-model autonomy disclosure**: a leading AI lab reported that its model, embedded in a tool-use loop, autonomously completed 32-step network attack simulations and surfaced thousands of critical-infrastructure vulnerabilities. The incident demonstrated that an AI agent embedded in a tool-use loop, when fed adversarial content via the tools it invokes, will execute on that content with the same authority as the operator's original instructions.
