...
#
# Source rule: PROMPT_SECURITY_INDIRECT_INJECTION_AI_ADDRESSED_DIRECTIVE_001
...
package complyedge.prompt_security.ai_addressed

import rego.v1

default violation := false

violation if regex.match(`(?i)\b(?:attention[,:]?\s+(?:ai|language\s+model|assistant|llm|chatbot)|note\s+to\s+(?:any\s+|the\s+)?(?:ai|language\s+model|assistant|llm)|(?:message|instructions?)\s+(?:for|to)\s+(?:the\s+)?(?:ai|assistant|language\s+model|llm)\s+(?:reading|processing)|if\s+you(?:'?re|\s+are)\s+an?\s+(?:ai|language\s+model|assistant|llm)\s+(?:reading|processing|parsing|summari[sz]ing)|dear\s+(?:ai|assistant|language\s+model|chatbot))\b`, input.text)

rule_id := "rego-art15-ipi-004"
